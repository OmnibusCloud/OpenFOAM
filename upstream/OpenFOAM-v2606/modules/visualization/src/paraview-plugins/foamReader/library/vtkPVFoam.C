...
)
{
    ...
    // Set text properties
    if (vtkTextProperty* tprop = txt->GetTextProperty())
    {
    ...
    // Set position
    if (vtkCoordinate* coord = txt->GetPositionCoordinate())
    {
...
Foam::word Foam::vtkPVFoam::getFoamAreaName(const std::string& longName)
{
    word areaName;

    // From "area/shell" get "shell"
    if (longName.find('/') != std::string::npos)
    {
        areaName = getFoamName(longName);
    }
    // But treat "area" like "area/region0"
    if (areaName.empty())
    {
        areaName = polyMesh::defaultRegion;
    }
    ...
    rangePointSets_("pointSet")
{
    ...
    // Check for embedded '{region}' inside of
    // "../case{region}.ext" or ../case{region}"

    {
        const auto beg = FileName.find_last_of("/{");
        const auto end = FileName.find('}', beg);

        if
        (
            beg != std::string::npos && end != std::string::npos
         && FileName[beg] == '{'
         && (end+1 == FileName.size() || FileName[end+1] == '.')
        )
        {
            meshRegion_ = FileName.substr(beg+1, end-beg-1);

            if (meshRegion_.empty())
            {
                meshRegion_ = polyMesh::defaultRegion;
            }
            meshDir_ = polyMesh::meshDir(meshRegion_);
        }
    }
...
void Foam::vtkPVFoam::updateInfo()
{
    ...
    // Fields selection
    {
        ...
        // Regular (volume, point fields, ...)
        // For this time and mesh region
        ...
        // Volume fields - includes save/restore of selected
        updateInfoContinuumFields
        (
    ...
    // Area fields - includes save/restore of selected
    updateInfoAreaFields(reader_->GetAreaFieldSelection());
...
)
{
    ...
    // Update the OpenFOAM mesh
    {
        ...
        if (!volMeshPtr_)
        {
            DebugInfo
                ...
                << " at time=" << runTime.timeName() << nl;

            volMeshPtr_ = std::make_unique<fvMesh>
            (
                IOobject
                (
                    meshRegion_,
        ...
        {
            const List<label> areaIds
            (
                rangeArea_.intersection(selectedPartIds_)
            );

            wordHashSet selectedNames;

            for (const label partId : areaIds)
            {
                const auto& longName = selectedPartIds_[partId];
                const word areaName = getFoamAreaName(longName);
                selectedNames.insert(areaName);
            }

            // Remove any deselected meshes
            wordHashSet deselected(areaMeshes_.toc());
            deselected -= selectedNames;
            for (const word& name : deselected)
            {
                areaMeshes_.remove(name);
            }

            // Create new ones if required
            for (const word& areaName : selectedNames)
            {
                if (!areaMeshes_.contains(areaName))
                {
                    auto ptr = autoPtr<faMesh>::New(areaName, *volMeshPtr_);
                    areaMeshes_.insert(areaName, std::move(ptr));
                }
            }
        }
        else
        {
            // No area meshes selected
            areaMeshes_.clear();
        }
    ...
    // Update fields
    {
        ...
        if (volMeshPtr_)
        {
            objects = IOobjectList(*volMeshPtr_, runTime.timeName());
        }

        convertVolFields(objects);
        convertPointFields(objects);
    }
...
)
{
    ...
    if (show && volMeshPtr_)
    {
        ...
        forAll(pbm, patchi)
        {
            displayZoneI += Foam::min(MAXPATCHZONES, nZones[patchi]);
        }
