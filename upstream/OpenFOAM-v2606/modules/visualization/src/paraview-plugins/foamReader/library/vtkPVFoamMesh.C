    ...
    for (const auto partId : partIds)
    {
        ...
        if (longName.starts_with("group/"))
        {
            ...
            for (const auto id : patchIds)
            {
                ...
                if (n)
                {
                    vtpData.additionalIds().push_back(id);
                    sz += n;
                }
            }
            ...
            for (const auto id : vtpData.additionalIds())
            {
                ...
                forAll(pp, i)
                {
                    faceLabels.push_back(pp.start()+i);
                }
            }
...
void Foam::vtkPVFoam::convertMeshArea()
{
    ...
    for (auto partId : partIds)
    {
        ...
        const auto* meshptr = areaMeshes_.get(areaName);
        if (!meshptr)
        {
            cachedVtp_.erase(longName);
            continue;
        }
        const faMesh& mesh = *meshptr;
...
void Foam::vtkPVFoam::convertMeshPointZones()
{
    ...
    for (const auto partId : partIds)
    {
        ...
        auto vtkpoints = NewPoints(pointMap.size());

        forAll(pointMap, pointi)
        {
            const auto& p = points[pointMap[pointi]];
            vtkpoints->SetPoint(pointi, p.cdata());
        }
