...
(
    vtkIdType count,
    const bool narrow = (sizeof(float) == sizeof(Foam::scalar))
)
{
    auto vtkpoints = vtkSmartPointer<vtkPoints>::New();
    if (narrow || (sizeof(float) == sizeof(Foam::scalar)))
    {
...
)
{
    ...
    for (auto partId : range)
    {
        ...
        const labelUList& blockLabels = blockDef.blockShape();

        auto vtkpoints = NewPoints(blockLabels.size());
        ...
        forAll(blockLabels, pointi)
        {
            ...
            (
                pointi,
                blkPoints[blockLabels[pointi]].cdata()
            );
...
)
{
    ...
    DynamicList<vtkIdType> pointIds(nEdgeDivs + 1);
    ...
    for (auto partId : range)
    {
        ...
        for (const blockDescriptor& blockDef : blkMesh)
        {
            ...
            if (foundEdgei != -1)
            {
                ...
                auto vtkpoints = NewPoints(edgePoints.size());

                forAll(edgePoints, pointi)
                {
                    vtkpoints->SetPoint(pointi, edgePoints[pointi].cdata());
...
)
{
    ...
    {
        auto vtkpoints = NewPoints(blkPoints.size());

        forAll(blkPoints, pointi)
        {
            vtkpoints->SetPoint(pointi, blkPoints[pointi].cdata());
        }
