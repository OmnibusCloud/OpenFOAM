...
)
{
    ...
    {
        if (UPstream::master() || obj.parallel())
        {
...
template<class Type>
static void updateActors(UPtrList<Type>& objects, const scalar position)
{
    for (Type& obj : objects)
    {
...
template<class Type>
static void cleanup(UPtrList<Type>& objects)
{
    for (Type& obj : objects)
    {
        obj.clear();
...
)
{
    ...
    if (debug || log)
    {
        ...
        if (controller)
        {
            Info<< " (" << controller->GetNumberOfProcesses()
                << " processes)";
        }
        Info<< endl;
    }
